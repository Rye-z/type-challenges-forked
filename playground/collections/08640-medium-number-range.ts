...
// 思路：先获取 [0, start] 和 [0, end] 的联合类型，然后取交集
// 注意：number extends ‘具体数字’ 是 false，因为具体数字更为具体

type Utils<T extends number, U extends number[] = [], N = T> =
  U['length'] extends T
    ? N
    : Utils<T, [0, ...U], U['length'] | N>

type a = Utils<2>
// 包含 2 本身，Exclude 之后需要将 2 补回去
type NumberRange<L extends number, H extends number> = L | Exclude<Utils<H>, Utils<L>>
